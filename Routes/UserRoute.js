...
router.put('/user/:id',  async (req, res) => {
  ...
  const { 
    ...
    languagesKnown, caste, subcaste, gotram, raasi, star, status, deletereason, suspendreason,
...
router.get('/bureau/:bureauId/:gender', async (req, res) => {
  ...
  try {
    ...
    const filter = { gender, bureauId };
    for (let i = 1; i <= 8; i++) {
      filter[`step${i}`] = 1;
    }

    // Fetch users matching the filter and sort them by _id in descending order
    const users = await User.find(filter).sort({ _id: -1 });

    if (users.length === 0) {
      return res.status(404).json({
        message: `No users found with gender ${gender}, bureauId ${bureauId}, and all steps 1 to 8 completed.`,
      });
    }
...
router.get('/bureaupendinguser/:bureauId/:gender', async (req, res) => {
  ...
  try {
    ...
    const query = { gender, bureauId, $or: [] };

    // Add conditions to fetch users with any step (1-8) missing OR set to 0
    for (let i = 1; i <= 8; i++) {
      query.$or.push({ [`step${i}`]: 0 }, { [`step${i}`]: { $exists: false } });
    }
